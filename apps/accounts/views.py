...
# ── Documents ─────────────────────────────────────────────────────────────────
# Replace the entire documents section in apps/accounts/views.py with this.
# All five views map to the five URL names in urls.py:
#   documents_view       → accounts:documents
#   document_upload      → accounts:doc_upload
#   document_download    → accounts:doc_download
#   document_delete      → accounts:doc_delete
#   document_set_primary → accounts:doc_primary
#
# UserDocument fields used by the template:
#   pk, label, doc_type, is_primary, mime_type, file_name,
#   uploaded_at, size_display() [method], file_data [BinaryField]

@login_required
def documents_view(request):
    docs = UserDocument.objects.filter(user=request.user)
    grouped = {
        dt: {"label": dt_label, "docs": [d for d in docs if d.doc_type == dt]}
...
def document_upload(request):
    ...
    # For CV: newest upload becomes primary, demote old primaries
    if doc_type == "cv":
    ...
    if doc_type == "cv":
        _sync_cv_to_pipeline(request.user, file_data, f.name)
...
def document_set_primary(request, doc_id):
    doc = get_object_or_404(UserDocument, pk=doc_id, user=request.user)
    # Demote all primaries of same type for this user
    UserDocument.objects.filter(
        user=request.user, doc_type=doc.doc_type, is_primary=True
    ).update(is_primary=False)
    doc.is_primary = True
    doc.save(update_fields=["is_primary"])
    if doc.doc_type == "cv":
